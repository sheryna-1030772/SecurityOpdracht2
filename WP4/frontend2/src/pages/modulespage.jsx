...
function Module({ modules, onModulePress }) {
    return (
        <View style={styles.section}>
            {Array.isArray(modules) ? modules.map((module, index) => (
                <Pressable
                    key={index}
                    onPress={() => module.is_unlocked && onModulePress(module.id)}
                    style={({ pressed }) => [styles.button, { opacity: pressed ? 0.5 : 1, backgroundColor: module.is_unlocked ? 'white' : 'lightgray' }]}
                    disabled={!module.is_unlocked}
                >
                    <Text style={styles.buttonText}>{module.module_name}</Text>
                    <Text>Beschrijving: {module.description}</Text>
                    {module.is_unlocked ? (
                        <Text style={styles.unlockedText}>Ontgrendeld</Text>
                    ) : (
                        <Text style={styles.lockedText}>Vergrendeld</Text>
                    )}
                </Pressable>
            )) : <Text>Geen modules beschikbaar</Text>}
        </View>
    );
}
...
function ModulesApp() {
    ...
    useEffect(() => {
        const fetchModules = async () => {
            try {
                const token = await AsyncStorage.getItem('userToken');
                const response = await fetch(`${backendUrl}/api/modules=${instance_id}`, {
                    headers: {
                ...
                if (!response.ok) {
                    if (response.status === 401) {
                        ...
                        console.error('Modules ophalen mislukt:', response.statusText);
                    }
                    return;
                }
    ...
    const handleModulePress = (module_id) => {
        navigation.navigate("Activities", { module_id });
    };
        ...
        <View style={Platform.OS === "android" ? styles.container : styles.containerWeb}>
            ...
            <ScrollView contentContainerStyle={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
                <View style={Platform.OS === "android" ? styles.contentPage : styles.contentPageWeb}>
                    <Module modules={modules} onModulePress={handleModulePress} />
                </View>
                            <TouchableOpacity style={styles.backButton} onPress={() => navigation.goBack()}>
                <Text style={styles.backButtonText}>Terug naar Instanties</Text>
            </TouchableOpacity>
            </ScrollView>
            <Footer />
        </View>
